...
mod ed25519_benches {
    ...
    fn verify_batch_signatures(c: &mut Criterion) {
        ...
        static BATCH_SIZES: [usize; 1] = [100];
    ...
    #[cfg(feature = "batch")]
    fn verify_batch_different_messages(c: &mut Criterion) {
        use ed25519_dalek::verify_batch;

        static BATCH_SIZES: [usize; 1] = [100];

        // Benchmark batch verification for all the above batch sizes
        let mut group = c.benchmark_group("Ed25519 batch signature (different) verification");
        for size in BATCH_SIZES {
            let name = format!("Aggregated");
            group.bench_function(name, |b| {
                let mut csprng: ThreadRng = thread_rng();
                ...
                let signatures: Vec<Signature> =
                     keypairs.iter().
                     zip(msgs.iter())
                     .map(|(key, msg)| key.sign(msg)).collect();
                let verifying_keys: Vec<_> =
                     keypairs.iter().map(|key| key.verifying_key()).collect();
                b.iter(|| verify_batch(&msgs[..], &signatures[..], &verifying_keys[..]));
            });
        }
    }
    ...
    criterion_group! {
        ...
        targets =
            ...
            verify_batch_different_messages,
          //  key_generation,
    }
}
