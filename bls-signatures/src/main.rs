...
macro_rules! measure {
 ...
 fn main() {
    ...
    println!("/********* Experiments with different messages ************/");
...
//    //   println!("\nserialized Signature: {:?}", serialized_signature);
    ...
    //   });
      ...
      measure!("verification messages", num_messages, {
          let messages = messages.par_iter().map(|r| &r[..]).collect::<Vec<_>>();
          assert!(verify_messages(&aggregated_signature, &messages[..], &public_keys));
      });

    println!("\n\n/********* Experiments with same message ************/");
    // generate messages
